<?php
	...
	if(isset($_POST['acao'])){
		...
		$sql = ConexaoBD::conexao();
		$sql = $sql->prepare("UPDATE `produto` SET `nome`= ?,`quantidade`= ?,`preco_custo`= ?,`preco_venda`= ?,`categoria`= ? WHERE `id` = $id");
		$sql->execute(array($nome,$quantidade,$precoCusto,$preco_venda,$categoria));
